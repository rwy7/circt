...
void InstanceOp::build(OpBuilder &builder, OperationState &result,
  ...
  result.addAttribute("moduleName", resultType.getModuleNameAttr());
...
// static
Type InstanceSubOp::inferReturnType(ValueRange operands,
                                    ArrayRef<NamedAttribute> attrs,
                                    std::optional<Location> loc) {
  auto instanceType = cast<InstanceType>(operands[0].getType());
  ...
  if (instanceType.getNumElements() <= index)
    return emitInferRetTypeError(loc, "element index is greater than the "
                                      "number of fields in the instance type");
...
void InstanceSubOp::print(OpAsmPrinter &p) {
  auto input = getInput();
  auto instanceType = input.getType();
  p << ' ' << input << "[";
  p.printKeywordOrString(instanceType.getElement(getIndex()).name);
  p << "]";
...
ParseResult InstanceSubOp::parse(OpAsmParser &parser, OperationState &result) {
  auto *context = parser.getContext();
  ...
  auto instanceType = dyn_cast<InstanceType>(inputType);
  if (!instanceType)
    return parser.emitError(parser.getNameLoc(),
                            "input must be !firrtl.instance type, got ")
           << inputType;

  auto index = instanceType.getElementIndex(fieldName);
  if (!index)
    return parser.emitError(parser.getNameLoc(),
                            "unknown field " + fieldName + " in instance type ")
           << instanceType;
